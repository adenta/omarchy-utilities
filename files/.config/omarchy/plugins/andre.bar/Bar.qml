...
Item {
  ...
  // Plugin bars are created asynchronously and receive host properties from
  // shell.configureBar() after construction, so these cannot be `required`.
  ...
  // Keep the bar anchored. Position can still be changed deliberately through
  // `omarchy bar position`, but dragging empty bar space cannot move it.
  readonly property bool positionLocked: true
