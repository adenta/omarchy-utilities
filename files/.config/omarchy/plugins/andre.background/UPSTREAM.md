# Background upstream maintenance

Reviewed against installed Omarchy 4.0.4-1 on 2026-09-17.

The only functional customization removes empty-desktop double-click wallpaper/theme actions. The remaining renderer matches installed stock.

Five seconds after each component instance starts, its own checker compares sorted
stock filenames and SHA-256 hashes in `/usr/share/omarchy/shell/plugins/background`
...
```sh
(cd /usr/share/omarchy/shell/plugins/background && find . -type f -print0 | LC_ALL=C sort -z | xargs -0 -r sha256sum | sha256sum | cut -d ' ' -f 1) > upstream.sha256
node tests/run.cjs
