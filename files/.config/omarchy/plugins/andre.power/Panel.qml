...
Panel {
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onTextKey: function(text) {
        if (text === "1" || text === "2" || text === "3") historyGraph.selectRange(Number(text) - 1)
      }
      ...
      Column {
        ...
        HistoryGraph {
          id: historyGraph
          width: parent.width
          opened: root.opened && root.batteryPresent
  ...
  Process {
    ...
    onExited: function(code, status) {
      if (code !== 0 || status !== 0) console.warn("omarchy power: upstream check/notification process failed", code, status)
    }
  }
}
