...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'power panel-upstream-'));
try {
  ...
  notification('Custom power panel needs review');
  notification('Custom power panel needs review'); // No suppression on later startups.
  ...
  fs.writeFileSync(baseline, 'invalid'); assert.notEqual(run().status, 0); notification('Power update check failed');
  fs.unlinkSync(baseline); notification('Power update check failed');
  fs.writeFileSync(baseline, hash); fs.rmSync(stock, { recursive: true }); notification('Power update check failed');
  fs.mkdirSync(stock); fs.writeFileSync(path.join(stock, 'Service.qml'), 'original');
  stub('find', 'exit 1\n'); notification('Power update check failed'); fs.unlinkSync(path.join(bin, 'find'));
  stub('find', 'sleep 10\n');
  const started = Date.now(); notification('Power update check failed');
