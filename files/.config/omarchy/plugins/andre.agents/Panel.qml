...
Panel {
  ...
  Process {
    id: upstreamCheck
    command: ["bash", Qt.resolvedUrl("check-upstream").toString().replace("file://", "")]
  ...
  component LimitRow: Column {
    ...
    Item {
      ...
      Text {
        ...
        text: limitRow.window ? limitRow.window.title + (limitRow.pace ? " · " + limitRow.pace.label : "") : ""
        color: root.foreground
        font.family: root.fontFamily
        font.pixelSize: Style.font.body
        elide: Text.ElideRight
  ...
  component Meter: Item {
    ...
    Rectangle {
      visible: meter.paceTarget >= 0
      width: Style.space(4)
      height: meter.thickness + Style.space(6)
      x: root.clamp(meter.width * meter.paceTarget - width / 2, 0, Math.max(0, meter.width - width))
      anchors.verticalCenter: parent.verticalCenter
      color: root.surface
      Rectangle {
        anchors.centerIn: parent
