...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'idle-upstream-'));
try {
  ...
  notification('Custom idle service needs review');
  notification('Custom idle service needs review'); // No suppression on later startups.
  ...
  fs.writeFileSync(baseline, 'invalid'); assert.notEqual(run().status, 0); notification('Idle update check failed');
  fs.unlinkSync(baseline); notification('Idle update check failed');
  fs.writeFileSync(baseline, hash); fs.rmSync(stock, { recursive: true }); notification('Idle update check failed');
  fs.mkdirSync(stock); fs.writeFileSync(path.join(stock, 'Service.qml'), 'original');
  stub('find', 'exit 1\n'); notification('Idle update check failed'); fs.unlinkSync(path.join(bin, 'find'));
  stub('find', 'sleep 10\n');
  const started = Date.now(); notification('Idle update check failed');
